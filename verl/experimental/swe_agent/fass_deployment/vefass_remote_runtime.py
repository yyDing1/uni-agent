...
class RemoteRuntime(AbstractRuntime):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.logger = get_logger("rex-runtime", run_id)
    ...
    async def is_alive(self, *, timeout: float | None = None) -> IsAliveResponse:
        ...
        try:
            async with aiohttp.ClientSession(
                connector=aiohttp.TCPConnector(force_close=True), proxy="http://sys-proxy-rd-relay.byted.org:8118"
            ) as session:
    ...
    async def _request(
        ...
        while retry_count <= num_retries:
            try:
                # TODO(dyy): mv proxy setting to config
                ...
                if retry_count <= num_retries:
                    ...
                    self.logger.error(f"Error making request {request_id} after {retry_count} retries: {e}")
                self.logger.error(f"Hit retries limit {num_retries}, fail after {retry_count} retries.")
